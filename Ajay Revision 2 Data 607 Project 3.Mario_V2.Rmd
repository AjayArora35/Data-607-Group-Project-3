...
```{r}
grid21 <- ggplot(data = data2C,aes(x=reorder(data2C$Keyword, data2C$LinkedIn), y=data2C$LinkedIn, fill = viridis(15),)) +theme(legend.position = "none", axis.text.y = element_text(size=8), axis.text.x = element_text(size=8)) +
  geom_bar(stat = "identity") + 
  geom_label(aes(label=data2C$LinkedIn), position = position_dodge(width = 0.5), size = 3, label.padding = unit(0.3, "lines"), label.size = 0.15, inherit.aes = TRUE)+
...
```{r}
grid22 <- ggplot(data = data2C,aes(x=reorder(data2C$Keyword, data2C$Indeed), y=data2C$Indeed, fill = magma(15),)) +theme(legend.position = "none", axis.text.y = element_text(size=8), axis.text.x = element_text(size=8)) +
  geom_bar(stat = "identity") + 
  geom_label(aes(label=data2C$Indeed), position = position_dodge(width = 0.5), size = 3, label.padding = unit(0.3, "lines"), label.size = 0.15, inherit.aes = TRUE)+
...
```{r}
grid23 <- ggplot(data = data2C,aes(x=reorder(data2C$Keyword, data2C$Monster), y=data2C$Monster, fill = data2C$Keyword,)) +theme(legend.position = "none", axis.text.y = element_text(size=8), axis.text.x = element_text(size=8)) +
  geom_bar(stat = "identity") + 
  geom_label(aes(label=data2C$Monster), position = position_dodge(width = 0.5), size = 3, label.padding = unit(0.3, "lines"), label.size = 0.15, inherit.aes = TRUE)+
...
```{r}
grid24 <- ggplot(data = data2C,aes(x=reorder(data2C$Keyword, data2C$SimplyHired), y=data2C$SimplyHired, fill = plasma(15),)) +theme(legend.position = "none", axis.text.y = element_text(size=8), axis.text.x = element_text(size=8)) +
  geom_bar(stat = "identity") + 
  geom_label(aes(label=data2C$SimplyHired), position = position_dodge(width = 0.5), size = 3, label.padding = unit(0.3, "lines"), label.size = 0.15, inherit.aes = TRUE)+
...
```{r}
Total_comp <- data2C %>% mutate(Total_Count = LinkedIn + Indeed + Monster + SimplyHired)

plot_comp <- ggplot(data = Total_comp,aes(x=reorder(Total_comp$Keyword, Total_comp$Total_Count), y=Total_comp$Total_Count, fill = magma(15),)) + 
...
#Substitute "machine learning" with "ml" so it fits in the wordcloud below
wordcloud(words = sub("machine learning", "ml", Total_comp$Keyword ), freq = Total_comp$Total_Count,
          max.words=200, random.order=FALSE, rot.per=0.35, 
          colors=brewer.pal(7, "Dark2"))
```
